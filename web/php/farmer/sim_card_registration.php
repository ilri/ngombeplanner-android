   ...
   private $alertHandler;
	...
	public function __construct() {
      ...
      $this->alertHandler = new AlertHandler($this->ROOT, $this->database, $this->logHandler);
		...
		$query="SELECT `sim_card_sn`,`id` FROM `farmer` WHERE `mobile_no` = '{$this->jsonObject['oldMobileNumber']}'";
		...
		if(sizeOf($result) == 1) {
         ...
         $appType = "JavaME";
         if(isset($this->jsonObject['newSimCardSN']) && strlen($this->jsonObject['newSimCardSN']) > 0){//means that it's coming from the JavaME app
            $appType = "Android";
         }
         
         $query = "INSERT INTO number_change_log(farmer_id, old_number, new_number, app_used) VALUES({$result[0]['id']}, '{$this->jsonObject['oldMobileNumber']}', '{$this->jsonObject['newMobileNumber']}', '{$appType}')";
         $this->database->runMySQLQuery($query, false);
         
         $this->alertHandler->sendNumberChangeAlert($result[0]['id'], $this->jsonObject['oldMobileNumber'], $this->jsonObject['newMobileNumber']);
