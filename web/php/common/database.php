...
class DatabaseHandler {
	...
	public function __construct($rootDir = "../../") {
      $this->ROOT = $rootDir;
		...
		$this->logHandler = new LogHandler($rootDir);
    ...
    private function connectToDatabase() {
		if($this->settings != null) {
         ...
         if($mysqlError!="") {
			 ...
			 $this->logHandler->log(3, $this->TAG, "Successfully connected to ".$this->settings['db_name']);
		 }
      }
   }
   
   public function runMySQLQuery($query, $getResult, $errorResponse = "") {
	  $this->logHandler->log(4, $this->TAG,"running query ".$query);
